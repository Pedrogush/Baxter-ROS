...
class noise_canceller(object):
	def __init__(self, a, b):
		self.a = a
		self.b = b
		self.F = [[0 for i in range(a)] for j in range(b)]
		self.atA = 0
		self.atB = 0
                self.V = [[[] for i in range(a)] for j in range(b)]
                self.size = [[0 for i in range(a)] for j in range(b)]
                while self.atA < a:
			while self.atB < b-1:
			    self.atB = self.atB + 1
		...
		self.MeansV = [0]*17
		self.MeanDiffDen = [0]*17
		self.MeanDiff = [0]*17
		self.HighDiff = [0]*17
		self.LowDiff = [100]*17
		self.Ef1 = []
		self.Ef1_listLoc = [[0 for j in range(a)] for k in range(b)]
		self.numbercount = 0
		self.decimals = 1
		self.N_num = [[0 for i in range(a)] for j in range(b)]
                self.set = [[0 for i in range(a)] for j in range(b)]
		self.allocation = [[0 for i in range(a)] for j in range(b)]
		self.allocationmax = 0
                self.i=0
		self.yetanothervariable = 0
		self.precedingstring = 0
		self.l=-1
		self.iEMode = 0
...
# COMPLETELY WRONG< REDO!
		...
		while self.atB< self.b:
			while self.atA< self.a:
				if self.atA>self.atB:
					while self.i<17:
							if -30<self.Ef1[self.Ef1_listLoc[self.atA][self.atB]][self.i]<30:
								self.Means[self.atA][self.atB][self.i] = self.Ef1[self.Ef1_listLoc[self.atA][self.atB]][self.i]
							if -30<self.Ef1[self.Ef1_listLoc[self.atA][self.atB]][17+self.i]<30:
								self.Lows[self.atA][self.atB][self.i] = self.Ef1[self.Ef1_listLoc[self.atA][self.atB]][17+self.i]
							if -30<self.Ef1[self.Ef1_listLoc[self.atA][self.atB]][34+self.i]<30:
								self.Highs[self.atA][self.atB][self.i] = self.Ef1[self.Ef1_listLoc[self.atA][self.atB]][34+self.i]
						        self.i=self.i+1
				self.l=0
				self.i=0
				self.atA = self.atA+1
	...
	def treat_variables(self):
		while self.atB<self.b:
			while self.atA<self.a:
				if self.atA>self.atB:
					while self.i<17:
						self.MeanDiff[self.i] = self.MeanDiff[self.i] + self.Means[self.atA][self.atB][self.i]
						self.MeanDiffDen[self.i] = self.MeanDiffDen[self.i] + 1
		                                if self.HighDiff[self.i] < self.Highs[self.atA][self.atB][self.i]:
							self.HighDiff[self.i] = self.Highs[self.atA][self.atB][self.i]
						if self.LowDiff[self.i] > self.Lows[self.atA][self.atB][self.i]:
							self.LowDiff[self.i] = self.Lows[self.atA][self.atB][self.i]
						self.i = self.i +1
					self.i = 0
					self.atA = self.atA+1
			self.atA=0
	...
	def readmatrix(self):
		...
		while self.atA< self.a:
			while self.atB< self.b:
				if self.atB>self.atA:
	       				print self.Ef1[self.atA][self.atB]
				self.atB = self.atB+1
			self.atB = 0
			self.atA = self.atA+1
		self.atA=0
